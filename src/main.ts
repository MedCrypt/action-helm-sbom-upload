...
export async function run(): Promise<void> {
  ...
  const productVersionName: string = core.getInput(
  ...
  const shouldCreate = core.getBooleanInput(
  ...
  console.log(
    `Resolving product (${productName}) and version (${productVersionName})...`
  )
  const foundProducts = allProducts.filter(
    p => p.getName().toLowerCase() == productName.toLowerCase()
  )
  let foundOrCreatedProduct: OrganizationProduct | undefined = undefined
  if (foundProducts.length == 0) {
    if (!shouldCreate) {
      core.setFailed(
        `Unable to locate product ${productName}, and create-product-and-version-if-missing is false.`
      )
      return
    }
  ...
  const allVersions = await ListAllVersionsOfProduct(
    foundOrCreatedProduct.getId(),
    callInfo
  )
  const foundVersions = allVersions.filter(
    v =>
      v.getRawVersionString().toLowerCase() == productVersionName.toLowerCase()
  )
  ...
  if (foundVersions.length == 0) {
    ...
    console.log(
      `Creating version ${productVersionName} for product ${productName}...`
    )
    foundOrCreatedVersion = await CreateProductVersion(
      foundOrCreatedProduct.getId(),
      productVersionName,
      callInfo
    )
  ...
  const sbomUploadResponse = await UploadSbom(
    foundOrCreatedVersion.getId(),
    fileName,
    fileReadResults,
    callInfo
  )
  console.log(`SBOM uploaded successfully.`)
  sbomUploadResponse.statusMessages.forEach(msg =>
...
export function UuidBytesToString(
  uuidBytes: Uint8Array | string | undefined
): string | null
export function UuidBytesToString(
  uuidBytes: Uint8Array | string | undefined
): string | null {
...
const GetDefaultOrganization = async (
  ...
  const orgResponse = await DoWebApiPostRequest(
    'listorganizations',
    listOrganizations,
    ListOrganizations,
    callInfo
  ...
  if (orgs == undefined || orgs.length == 0) {
    throw Error(
      `Unable to determine default organization out of ${orgs?.length} possible candidates`
    )
  }
  return orgs[0]
}
...
const ListAllProducts = async (
  ...
  console.log(
    `Listing all products for org ${UuidBytesToString(organizationUuid?.getUuid())}`
  )
...
const ListAllVersionsOfProduct = async (
  ...
  console.log(
    `Listing all versions for product ${UuidBytesToString(productUuid?.getUuid())}`
  )
...
const UploadSbom = async (
  ...
  const submitSbomResponse = await DoWebApiPostRequest(
    'submitsbom',
    submitSbom,
    SubmitSbom,
    callInfo
  ...
  const response: UploadSbomResult = {
    statusMessages:
      submitSbomResponse.getResponse()?.getMetadata()?.getStatusMessageList() ??
      []
  }
  return response
}
